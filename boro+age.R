...
boro_age_arrests_query <- "
SELECT ARREST_BORO, AGE_GROUP, COUNT(*) AS boro_age_count 
...
boro_age_arrests_res <- dbSendQuery(conn, boro_age_arrests_query)
boro_age_arrests_df <- dbFetch(boro_age_arrests_res)
dbClearResult(boro_age_arrests_res)

dbDisconnect(conn)

#画图
boro_arrests <- boro_arrests_df$boro_count
boro_age_arrests <- boro_age_arrests_df$boro_age_count

#半径
total_radius <- 0.5
boro_radius <- boro_arrests * total_radius *2 / as.numeric(total_arrests_df * 2)
boro_NA_radius <- boro_age_arrests[1:4] * boro_radius[1] /  boro_arrests[1]
boro_B_radius <- boro_age_arrests[5:9] * boro_radius[2]  /  boro_arrests[2]
boro_K_radius <- boro_age_arrests[10:14] * boro_radius[3] /  boro_arrests[3]
boro_M_radius <- boro_age_arrests[15:19] * boro_radius[4] /  boro_arrests[4]
boro_Q_radius <- boro_age_arrests[20:24] * boro_radius[5] /  boro_arrests[5]
boro_S_radius <- boro_age_arrests[25:29] * boro_radius[6] /  boro_arrests[6]
...
#label of race
colors = c("#fc6666", "#fca14d", "#fdf233", "#50c800", "#80fdf2")
labels = c("18-24", "25-44", "45-64", "65+", "<18")

pushViewport(viewport(x = 0.52, y = 0.17, width = 0.2, height = 0.13))
